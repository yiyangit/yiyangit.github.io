...
(function(w) {
	...
	let t = Math.floor(Math.random() * 1000) % 4;while(t==bef)t = Math.floor(Math.random() * 1000) % 4;bef=t;
        let i = t + (loop ? 0 : 4);
        for (let j = 0; j < box.children.length; j++) {
            ...
            if (i === j) {
                ...
                r.notEmpty = true;
		let t = Math.floor(Math.random() * 1000) % 4;while(t==bef)t = Math.floor(Math.random() * 1000) % 4;
		bef=t;
                i = (Math.floor(j / 4) + 1) * 4 + t;
            } else {
                r.notEmpty = false;
            }
        }
